  ...
  const articleSections = data?.sections ?? [
    {
      ...
      title: "Key takeaways",
      bullets: [
        "A video production subscription gives you continuous access to a dedicated creative team for a fixed monthly cost. Predictable budget, predictable output.",
        "It eliminates repeated briefing, pricing, and onboarding cycles, making production faster and far more consistent.",
        "Compared to building an in-house team, subscriptions offer flexibility without long-term contracts, overhead, or hiring stress.",
        "The right subscription depends on how much content you need, how fast you need it, and how your team operates day to day.",
      ],
    },
    {
      id: "subscription",
      title: "Video production subscription",
      bullets: [
    ...
    },
    {
      ...
      bullets: [
        ...
        "A video creation subscription is designed for teams that need video consistently, not just once. Unlike one-off production, it keeps the pipeline warm, the workflow familiar, and the turnaround predictable.",
      ],
    },
  ];
  const introCopy =
        ...
        <div className="grid w-full items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16">
          <div className="max-w-2xl">
            {loading ? (
              <>
                ...
                <div className="space-y-4">
                  {skeletonLines.map((width) => (
                    <div key={width} className={`h-14 rounded-2xl bg-white/10 animate-pulse ${width}`} />
                  ))}
                </div>
              ...
              <>
                <p className="mb-5 text-sm tracking-wide text-white/45 sm:text-base">{category}</p>
                ...
                <div className="mt-2 max-w-3xl text-4xl font-semibold leading-[1.05] text-white sm:text-5xl lg:text-6xl">
                  A Practical Guide for Growing Brands
                </div>
              ...
              <div className="flex aspect-[1.02/1] w-full items-center justify-center px-8 text-center">
                <div>
                  <p className="text-sm uppercase tracking-[0.25em] text-white/50">Post not found</p>
                  <h2 className="mt-4 text-2xl font-semibold text-white">No blog entry is available for this slug.</h2>
                  <p className="mt-3 text-sm leading-6 text-white/70">
                    The page loaded, but there is no matching post data to render.
                  </p>
                </div>
              </div>
            )}
              ...
              {loading ? (
                ...
                <>
                  <div className="relative aspect-[1.02/1] w-full">
                    <SmartImage src={image} alt={title} fill className="object-cover opacity-80" />
                    ...
                    <div className="absolute left-1/2 top-1/2 w-[88%] -translate-x-1/2 -translate-y-1/2 rounded-3xl border border-white/12 bg-white/10 px-5 py-6 text-center backdrop-blur-md">
                      <p className="text-xs font-medium tracking-[0.24em] text-white/70">FEATURED STORY</p>
                      <h2 className="mt-3 text-2xl font-semibold leading-tight text-white sm:text-3xl">
                        {title}
                      </h2>
                      <p className="mt-2 text-sm leading-6 text-white/80">{excerpt}</p>
                    </div>
                  </div>
      ...
      <div className="rounded-[2rem] border border-white/5 bg-[#071b24]/92 p-6 shadow-[0_0_70px_rgba(0,169,189,0.08)] backdrop-blur-md sm:p-8 lg:p-12">
        ...
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-start">
          <article className="space-y-14">
            {articleSections.map((section, index) => {
              const points = section.points ?? section.bullets ?? [];
              const sectionId = section.id ?? `section-${index + 1}`;
          ...
          <aside className="lg:sticky lg:top-28">
            <div className="space-y-4 rounded-[1.8rem] bg-[#091725] p-5 shadow-[0_0_45px_rgba(0,0,0,0.28)]">
              ...
              <div className="flex items-center justify-center gap-4 text-white">
                <a href="#" className="text-xl transition hover:text-[#25d9e0]">🔗</a>
                <a href="#" className="text-xl transition hover:text-[#25d9e0]">in</a>
                <a href="#" className="text-xl transition hover:text-[#25d9e0]">◎</a>
                <a href="#" className="text-xl transition hover:text-[#25d9e0]">f</a>
              </div>
            </div>

            <div className="mt-5 rounded-[1.8rem] bg-[#091725] p-5 shadow-[0_0_45px_rgba(0,0,0,0.28)]">
              <div className="relative w-full">
                ...
                <div className="relative z-10 rounded-[1.4rem] border-[3px] border-[#006B77] bg-[#020617] p-4 shadow-[0_0_50px_rgba(0,107,119,0.3)]">
                  <div className="mb-4 flex h-[66px] items-center justify-between overflow-hidden rounded-2xl border border-white/10 bg-[#333b4d] px-4 shadow-inner">
                    <div data-calc-display className="font-sans text-2xl font-bold tracking-wide text-white">
                      $0
                    </div>
                  ...
                  <div className="grid grid-cols-4 gap-2.5">
                    {["7", "8", "9", "-", "4", "5", "6", "+", "1", "2", "3", "0", ".", "=", "C", "*"].map((key) => (
                      <div
                        key={key}
                        data-calc-key={key}
                        className="flex aspect-square items-center justify-center rounded-[14px] border-b-[4px] border-[#4b5563] bg-[#2a3441] text-lg font-bold text-[#f3f4f6] shadow-sm"
                      >
                        {key}
                      </div>
                    ))}
              ...
              <div className="mt-5 text-center">
                <h3 className="text-lg font-semibold text-white">Want to know the cost of your next video?</h3>
                <button className="mt-4 rounded-full bg-gradient-to-r from-[#006B77] to-[#2dd4bf] px-6 py-3 font-semibold text-white shadow-[0_0_20px_rgba(0,107,119,0.3)] transition hover:scale-105">
                  Calculate now
                </button>
              </div>
            </div>
