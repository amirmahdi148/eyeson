                ...
                <div
                    ...
                    onScroll={() => {
                        ...
                        frameRef.current = requestAnimationFrame(() => {
                            ...
                            const idx = Math.max(
                                0,
                                Math.min(serviceCards.length - 1, Math.round(scrollLeft / clientWidth)),
                            );
                    ...
                    {serviceCards.map((card) => (
                        ...
                        >
                            <div className="relative h-56 overflow-hidden rounded-2xl">
                                <SmartImage src={card.image} alt={card.title} fill className="object-cover" />
                                <div className="absolute inset-0 bg-gradient-to-t from-[#081521] via-transparent to-transparent" />
                            </div>
                            <div className="p-3 pb-4">
                                <h3 className="text-2xl font-bold text-white">{card.title}</h3>
                                <p className="mt-3 text-sm leading-7 text-[#c0d3df]">{card.description}</p>
                            </div>
                        </article>
                    ))}
                </div>
