...
export default function VideoPackagesSection() {
    ...
    return (
        <section className="overflow-x-hidden px-6 pb-24 pt-14 lg:px-20">
            <div className="mx-auto w-full ">
                <div className="w-full text-center md:hidden">
                    ...
                    <p className="mt-5 w-full text-sm leading-7 text-[#c4d6de]">
                ...
                <div className="w-full text-center hidden md:block">
                    ...
                    <p className="mt-5 w-full text-sm leading-7 text-[#c4d6de] md:text-base">
                        Every content strategy is different.That&apos;s why we offer flexible
                        editing packages designed to fit your volume, goals, and growth
                        stage.
                    </p>
                </div>
                    ...
                    >
                        {packs.map((pack) => (
                            <SwiperSlide key={`mobile-${pack.id}`} className="!h-auto">
                                <article
                                    className={`relative h-full rounded-3xl px-5 pb-4 pt-10 flex flex-col justify-between ${
                                        pack.featured
                                            ? "pt-12 border-[#1F5046] border-2 shadow-[0_0_22px_rgba(53,207,202,0.35)]"
                                ...
                                >
                                    ...
                                    <h3 className="bg-linear-to-r from-[#46B6A0] to-[#00A9BD] bg-clip-text text-[38px] font-bold leading-none text-transparent">
                                        {pack.name}
                                    </h3>
                                    ...
                                    <div className="mt-4 flex items-end gap-2">
                                        ...
                                        <span className="pb-2 text-base text-[#d5e3e8]">{pack.subtitle}</span>
                                    </div>
                            ...
                            {tag}
                                    ...
                                    <div className="flex items-center justify-center">
                                        <button
                                            className={`mt-6 w-[160px] text-white border px-4 py-2 text-sm font-semibold transition cursor-pointer ${
                                                pack.featured
                ...
                <div className="mt-12 hidden h-[610px] w-full items-end justify-center gap-6 md:flex">
                    {packs.map((pack) => (
                        <article
                            key={pack.id}
                            className={`relative  rounded-3xl  px-5 pt-10 pb-[16px]  flex flex-col justify-between ${
                                pack.featured
                                    ? "pt-12 border-[#1F5046] border-3 h-full shadow-[0_0_22px_rgba(53,207,202,0.35)] w-[500px]"
                                    : "border-[#144e62] bg-linear-to-b from-[#0B1F2A] to-[#15393E] w-[400px]"
                            }`}
                            style={
                                pack.featured
                                    ? {
                        ...
                        >
                            {pack.featured && pack.highlight ? (
                                <div className="absolute -top-4 left-2/7 -translate-x-1/2 rounded-full border border-[#66f0e6] bg-linear-to-r from-[#2F907F] to-[#033D4D] px-5 py-1 text-lg font-semibold text-[#e8ffff]">
                                    {pack.highlight}
                                </div>
                            ) : null}

                            <h3 className="text-3xl font-bold bg-linear-to-r from-[#46B6A0] to-[#00A9BD] bg-clip-text text-transparent ">{pack.name}</h3>
                            <p className="mt-2 text-xl text-[#d5e3e8]">{pack.videos}</p>
                        ...
                        {tag}
                            ...
                            <div className="flex items-center justify-center">
                                <button
                                    className={`mt-6 w-[131px]  text-white border px-4 py-2 text-sm font-semibold transition cursor-pointer ${
                                        pack.featured
                                            ? "border-[#61f4e8] bg-linear-to-l from-[#033D4D] to-[#2F907F] shadow-[0_0_19px_#46B6A080] rounded-[90px]"
                                            : "border-[#46B6A0] border-[0.3px] bg-[#0B1F2A]  rounded-xl"
                                    }`}
