...
export default function HeroHome() {
  ...
  return (
    <section className="relative w-full overflow-hidden pt-24 pb-16 sm:pt-28 sm:pb-20 lg:pt-36 lg:pb-32 bg-[radial-gradient(ellipse_1000px_600px_at_50%_-10%,rgba(0,169,189,0.12),transparent_70%),radial-gradient(ellipse_800px_400px_at_90%_25%,rgba(46,182,160,0.08),transparent_60%)]">
      {/* Ambient blobs */}
      ...
      <div className="relative mx-auto max-w-[1550px] px-4 sm:px-6 lg:px-8 xl:px-12 flex flex-col items-center gap-10 lg:gap-8 xl:gap-14 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="w-full lg:w-[42%] xl:w-[40%] order-2 lg:order-1 flex flex-col items-center text-center lg:items-start lg:text-left animate-fade-in">
          <p className="tracking-[0.2rem] text-white/60 text-[10px] sm:text-xs uppercase font-medium animate-slide-up">
            Product Launch Videos · Explainers · Social Media Content
          </p>

          <h1 className="mt-3 text-3xl sm:text-4xl lg:text-[2.6rem] xl:text-[3.2rem] font-extrabold leading-[1.12] text-white animate-slide-up max-w-xl">
            ...
            <span className="block">Animation & Editing Studio</span>
            <span className="mt-2 block text-white/90 text-lg sm:text-2xl lg:text-2xl xl:text-3xl font-semibold drop-shadow-md">
              Make Your Brand Impossible to Ignore.
            </span>
          </h1>

          <p className="mt-4 sm:mt-5 text-sm sm:text-base leading-relaxed text-white/70 max-w-lg font-light animate-fade-in">
          ...
          <div className="mt-6 sm:mt-8 flex flex-wrap gap-3 sm:gap-4 justify-center lg:justify-start w-full sm:w-auto animate-slide-up">
            <PrimaryButton text="Get Started" href="/contact" width="13rem" height="48px" />
            <SecondaryButton text="View Pricing" href="/pricing" width="13rem" height="48px" />
          </div>

          <p className="mt-5 sm:mt-6 text-xs text-white/40 tracking-wider">
      ...
      <div className="mt-14 sm:mt-18 lg:mt-24 px-4 max-w-5xl mx-auto flex flex-col items-center justify-center gap-6 text-white animate-fade-in">
        <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-10 md:gap-14">
            ...
            <div className="flex -space-x-2.5 isolate">
              {items.map((item, index) => (
                <div
                  ...
                  className={`h-9 w-9 sm:h-10 sm:w-10 rounded-full overflow-hidden border-2 cursor-pointer shadow-md transition-all duration-200 hover:scale-110 hover:-translate-y-1
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-lg sm:text-xl font-bold text-[#00E6D7]">1200+</span>
            <span className="text-xs sm:text-sm text-white/60">Projects Delivered</span>
          </div>

          <div className="hidden sm:block h-8 w-px bg-white/10" />

          <div className="flex items-baseline gap-2">
            <span className="text-lg sm:text-xl font-bold text-[#00E6D7]">25+</span>
            <span className="text-xs sm:text-sm text-white/60">Countries Served</span>
          </div>
        </div>
